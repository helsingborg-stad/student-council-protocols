...
class CacheBust
{
    ...
    {
        $jsonPath = STUDENTCOUNCILPROTOCOLS_PATH . apply_filters('StudentCouncilProtocols/Helper/CacheBust/RevManifestPath', 'dist/rev-manifest.json');

        if (file_exists($jsonPath)) {
            return json_decode(file_get_contents($jsonPath), true);
        } elseif (WP_DEBUG) {
            echo '<div style="color:red">Error: Assets not built. Go to ' . STUDENTCOUNCILPROTOCOLS_PATH . ' and run gulp. See '. STUDENTCOUNCILPROTOCOLS_PATH . 'README.md for more info.</div>';
        }
    }
}
